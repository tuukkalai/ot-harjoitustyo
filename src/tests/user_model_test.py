...
class TestUserModel(unittest.TestCase):
	...
	def test_create_user_with_short_username(self):
		...
		self.assertEqual(str(e.value), 'Username or password too short, min. 3 characters')
	...
	def test_create_user_with_mismatch_password(self):
		with pytest.raises(PasswordMismatchError) as e:
	...
	def test_create_user_that_already_exists(self):
		...
		with pytest.raises(UsernameAlreadyExistsError) as e:
			self.user_model.create_user(username, '1234', '1234')
		self.assertEqual(str(e.value), f'Username `{username}` already exists')
		created_users = self.user_model._get_all_users()
